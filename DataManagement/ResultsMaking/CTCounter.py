...
class CTCounter():
    ...
    def run() -> None:
        ...
        row_template = {ct: 0 for ct in CONTACT_TYPES}
        tables_ct = {
        tool: {row: row_template.copy() for row in CONTACT_TYPES} for tool in TOOLS
        }
        
        # Now convert the above into a dictionary of dataframes
        for tool in tables_ct.keys():
            tables_ct[tool] = pd.DataFrame(tables_ct[tool])
    ...
    def _write_tables(tables_ct: dict[pd.DataFrame]) -> None:
        for tool, df in tables_ct.items():
            path = os.path.join('ConfusionMatrices', f'{tool}_confusion_matrix.csv')
            df.to_csv(path)
